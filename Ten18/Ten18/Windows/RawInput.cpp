...
{
    UINT num = 0;
    Expect.NotEqualTo(UINT(-1)) = GetRawInputData(reinterpret_cast<HRAWINPUT>(lParam), RID_INPUT, nullptr, &num, sizeof(RAWINPUTHEADER));

    const UINT max = 1024;
    BYTE buffer[max];
    Ten18_ASSERT(num < max);

    Expect.EqualTo(UINT(num)) = GetRawInputData(reinterpret_cast<HRAWINPUT>(lParam), RID_INPUT, buffer, &num, sizeof(RAWINPUTHEADER));
...
void RawInput::Register(HWND hwnd)
{
    UINT num = 0;
    UINT rum = 0;
    Expect.NotEqualTo(UINT(-1)) = GetRawInputDeviceList(nullptr, &num, sizeof(RAWINPUTDEVICELIST));
    ...
    Expect.EqualTo(UINT(num)) = GetRawInputDeviceList(dls, &num, sizeof(RAWINPUTDEVICELIST));
    ...
    {         
        ...
        Expect.EqualTo(sizeof(rdi)) = GetRawInputDeviceInfo(dls[i].hDevice, RIDI_DEVICEINFO, &rdi, &size);
    ...
    Expect.EqualTo(TRUE) = RegisterRawInputDevices(rids, rum, sizeof(rids[0]));
}
