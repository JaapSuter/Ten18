...
GuidAndName* knownGuidNamesEnd = knownGuidNames + _countof(knownGuidNames);
...
static void LogAttributeValueByIndex(IMFAttributes& attr, DWORD index)
{
    ...
    Ten18_EXPECT.HR = attr.GetItemByIndex(index, &guid, &var);
...
{
	UINT32 count = 0;
    Ten18_EXPECT.HR = mediaType.GetCount(&count);
