...
COM::COMPtr<IMFMediaSource> TrySymbolicLink(const wchar_t* sl)
{
    ...
    Ten18_EXPECT.HR = MFCreateAttributes(attributes.AsTypedDoubleStar(), 2);
    Ten18_EXPECT.HR = attributes->SetGUID(MF_DEVSOURCE_ATTRIBUTE_SOURCE_TYPE, MF_DEVSOURCE_ATTRIBUTE_SOURCE_TYPE_VIDCAP_GUID);
    Ten18_EXPECT.HR = attributes->SetString(MF_DEVSOURCE_ATTRIBUTE_SOURCE_TYPE_VIDCAP_SYMBOLIC_LINK, sl);
...
void MediaFoundationCapture::Initialize()
{
    ...
    Ten18_EXPECT.HR = MFStartup(MF_VERSION, MFSTARTUP_LITE);
    ...
    COM::COMPtr<IMFAttributes> attributes;                
    Ten18_EXPECT.HR = MFCreateAttributes(attributes.AsTypedDoubleStar(), 1);        
    Ten18_EXPECT.HR = attributes->SetGUID(MF_DEVSOURCE_ATTRIBUTE_SOURCE_TYPE, MF_DEVSOURCE_ATTRIBUTE_SOURCE_TYPE_VIDCAP_GUID);
    
    Ten18_EXPECT.HR = MFEnumDeviceSources(attributes.Raw(), sActivates.FillArray(), sActivates.FillCount());
...
MediaFoundationCapture* MediaFoundationCapture::Get(int idx)
{
    ...
    else
    {
        ...
        Ten18_EXPECT.HR =  activate->ActivateObject(__uuidof(mediaSource.Raw()), mediaSource.AsVoidDoubleStar());
      ...
      mSymbolicLink(symbolic)
{
    ...
    Ten18_EXPECT.HR = MFCreateSourceReaderFromMediaSource(mMediaSource.Raw(), attributes.Raw(), mSourceReader.AsTypedDoubleStar());
...
void MediaFoundationCapture::ChooseBestMode(int width, int height)
{
    ...
    for (DWORD streamIdx = 0; SUCCEEDED(hr); ++streamIdx)
    {
        for (DWORD mediaTypeIdx = 0; ; ++mediaTypeIdx)
        {
            ...
            GUID majorType;
            Ten18_EXPECT.HR = mediaType->GetGUID(MF_MT_MAJOR_TYPE, &majorType);

            if (majorType != MFMediaType_Video)
            {
                Ten18_EXPECT.HR = mSourceReader->SetStreamSelection(streamIdx, FALSE);
                continue;
            }

            GUID subType;
            Ten18_EXPECT.HR = mediaType->GetGUID(MF_MT_SUBTYPE, &subType);
            
            BOOL isCompressedFormat = false;
            Ten18_EXPECT.HR = mediaType->IsCompressedFormat(&isCompressedFormat);

            UINT32 w = 0;
            UINT32 h = 0;
            Ten18_EXPECT.HR = MFGetAttributeSize(mediaType.Raw(), MF_MT_FRAME_SIZE, &w, &h);            
            ...
            Ten18_EXPECT.HR = MFGetAttributeRatio(mediaType.Raw(), MF_MT_FRAME_RATE_RANGE_MAX, &frameRateMinNumer, &frameRateMinDenom);
            Ten18_EXPECT.HR = MFGetAttributeRatio(mediaType.Raw(), MF_MT_FRAME_RATE_RANGE_MIN, &frameRateMaxNumer, &frameRateMaxDenom);
            Ten18_EXPECT.HR = MFGetAttributeRatio(mediaType.Raw(), MF_MT_FRAME_RATE, &frameRateNumer, &frameRateDenom);
            ...
            {
                mSize = dx::XMFLOAT2A(static_cast<float>(width), static_cast<float>(height));
                mBytesPerPixel = 4;
                mStreamIdx = streamIdx;
                fpsMax = fps;

                Ten18_EXPECT.HR = mSourceReader->SetCurrentMediaType(streamIdx, nullptr, mediaType.Raw());
            }
        }
    }

    Ten18_EXPECT.HR = mSourceReader->ReadSample(mStreamIdx, 0, nullptr, nullptr, nullptr, nullptr);
}
...
HRESULT MediaFoundationCapture::OnReadSample(HRESULT hr, DWORD, DWORD dwStreamFlags, LONGLONG, IMFSample* sample)
{
    ...
    { 
        ...
        Ten18_HOPE_FOR.HR = mSourceReader->ReadSample(mStreamIdx, 0, nullptr, nullptr, nullptr, nullptr);
    });
    ...
    for (DWORD b = 0; b < bufferCount; ++b)
    {
        ...
        else
        {
            ...
            OnExit oe([&] { Ten18_HOPE_FOR.HR = mb->Unlock(); });
