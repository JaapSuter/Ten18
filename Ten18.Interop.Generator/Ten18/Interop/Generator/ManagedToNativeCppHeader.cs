...
{
    partial class ManagedToNativeCppHeaderTemplate
    {
    ...
    class ManagedToNativeCppHeader
    {        
        public static void Generate(IEnumerable<string> nameSpaceNames, Type type, MethodInfo[] methodInfos, string dir)
        {
            var cppHeaderTemplate = new ManagedToNativeCppHeaderTemplate()
            {
                 NameSpaceNames = nameSpaceNames,
                 Type = type,
                 Guid = Guid.Parse(type.GetCustomAttributes(typeof(GuidAttribute), true).Cast<GuidAttribute>().First().Value),
